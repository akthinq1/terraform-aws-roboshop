# create target group for componet in bbackend ALB

resource "aws_lb_target_group" "main" {
  name                 = "${var.project_name}-${var.environment}-${var.component}" #roboshop-dev-{{component name}}
  port                 = local.tg_port
  protocol             = "HTTP"
  vpc_id               = local.vpc_id
...
# create launch template from catalogue instance instance +ebs volume
resource "aws_launch_template" "main" {
  ...
  # for volume specifications tags created by ASG
  tag_specifications {
...
# create aws auto scaling group ASG
...
resource "aws_lb_listener_rule" "main" {
  ...
  priority     = var.rule_priority #need to provide priority as per the component
